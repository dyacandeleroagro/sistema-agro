...
# Estilos adaptables a Modo Oscuro y Claro
st.markdown("""
    <style>
    .header-container {
        padding: 20px;
        ...
        with st.form("login_form"):
            ...
            if btn_login:
                if txt_user in USUARIOS_CREDENCIALES and USUARIOS_CREDENCIALES[txt_user]["password"] == txt_pass:
                    ...
                    st.session_state["nombre_usuario"] = USUARIOS_CREDENCIALES[txt_user]["nombre"]
                    st.success(f"¡Bienvenido {st.session_state['nombre_usuario']}!")
                    st.rerun()
                else:
                    st.error("❌ Usuario o contraseña incorrectos.")

# Si no está logueado frena la ejecución
...
if not os.path.exists("registro_empleados.csv"):
    # Precargamos los tres empleados fijos para que ya existan en la base de datos
...
# Todos acceden a rendiciones (el operario filtrado por su nombre)
lista_tabs.append("📋 RENDICIÓN POR OPERARIO")

if rol_actual == "Administrador":
    lista_tabs.append("🛡️ SEGUROS Y COBERTURAS")
    lista_tabs.append("🗄️ CONTROL DE ERRORES")
    ...
    with t_ing:
        ...
        with st.form("form_ingresos", clear_on_submit=True):
            ...
            if btn_ingreso and i_cliente and i_monto > 0:
                ...
                st.success("✔️ ¡Ingreso registrado!")
                st.rerun()
    ...
    with t_fac:
        ...
        with st.form("form_facturas", clear_on_submit=True):
            ...
            if btn_fac and proveedor and monto_orig > 0:
                ...
                st.success("✔️ ¡Gasto comercial registrado!")
                st.rerun()
            ...
            with st.form("form_pago_empleado"):
                if not df_empleados.empty:
                    ...
                    if btn_pago_emp and p_monto > 0:
                        ...
                        st.success("✔️ Movimiento registrado con éxito.")
                        st.rerun()
...
if t_rend:
    with t_rend:
        ...
        # FILTRO CRÍTICO: Si entra un operario, se bloquea la lista y solo ve su propio nombre
        if rol_actual == "Operario":
            lista_para_filtrar = [nombre_actual]
            st.info(f"Visualizando la cuenta de: **{nombre_actual}**")
        else:
            # Los admin pueden seleccionar a cualquiera de la lista
...
# ----------------------------------------------------
t_seg = obtener_tab("🛡️ SEGUROS Y COBERTURAS")
if t_seg:
    with t_seg:
        st.header("🛡️ Control de Pólizas y Seguros")
...
# ----------------------------------------------------
t_aud = obtener_tab("🗄️ CONTROL DE ERRORES")
if t_aud and rol_actual == "Administrador":
    with t_aud:
        st.header("🗄️ Panel exclusivo de borrado (Solo Administrador)")
        ...
        with sub_g:
            if not df_facturas.empty:
                for idx, fila in df_facturas.copy().iterrows():
                    ...
                    with c_b:
                        if st.button("🗑️ Borrar", key=f"b_fac_{fila['ID']}_{idx}"):
        ...
        with sub_i:
            if not df_ingresos.empty:
                for idx, fila in df_ingresos.copy().iterrows():
                    ...
                    with c_b:
                        if st.button("🗑️ Borrar", key=f"b_ing_{fila['ID_Ingreso']}_{idx}"):
        ...
        with sub_p:
            if not df_pagos_empleados.empty:
                for idx, fila in df_pagos_empleados.copy().iterrows():
                    ...
                    with c_b:
                        if st.button("🗑️ Borrar", key=f"b_emp_{fila['ID_Pago']}_{idx}"):
        ...
        with sub_s:
            if not df_seguros.empty:
                for idx, fila in df_seguros.copy().iterrows():
                    c_i, c_b = st.columns([6, 1])
                    with c_i: st.write(f"🛡️ {fila['Compañía']} | Bien: *{fila['Bien Asegurado']}* | **$ {fila['Monto Prima (ARS)']:,.2f}**")
                    with c_b:
                        if st.button("🗑️ Borrar", key=f"b_seg_{fila['ID_Seguro']}_{idx}"):
